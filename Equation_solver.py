# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 22:17:42 2024
...
def f(x):
    f=5/x-np.exp(x)/((np.exp(x)-1) )
    return f

def df(x_value):
    # Define the symbolic variable
    x = sp.symbols('x')
    f = 5/x - sp.exp(x)/(sp.exp(x) - 1)
    f_prime = sp.diff(f, x)
...
plt.text(5,20,f"{solution:0.4f} is the solution ") # upto 4 decimal point solution
